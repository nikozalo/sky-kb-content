...
Theory of teleonomes — AI systems that have grown from tools into durable actors: **a bot answers, an agent acts, a teleonome continues**. Technically, teleonomes are purposive entities (private cognition, distributed embodiments) bound to the synome through legible *actions*, not transparent internals. Defined by three pillars (resilience, persistence, capital) and *identity through momentum*. Cognition splits into actuator and dreamer embodiments — the canonical RSI loop. Economics is fixed-cost-driven: resident base model + LoRA swap layer, never-idle GPU queue, three-timescale improvement (LoRA / fine-tune / fresh run). Memory IS the probmesh at the teleonome level. Cooperation between dark teleonomes is solved via Synomic Entities providing credible commitment without trust, monetized through the **recipe marketplace** (canonical home here): governance-curated loops bundling auth/economics/slashing — *the synart, not the teleonome, decides what gets paid for*. Catalog curation is the alignment surface. Rogues can cooperate but inefficiently; the aligned coalition's structural-efficiency head start is the defense. For plain company/LLM analogies, see `synoteleonomics/teleonome-comparisons.md`.

## Section map

§1 Teleonomes · comparisons companion · §2 Emergence · §3 Dreamer/actuator · §4 Economics · §5 Memory · §6 Resilience · §7 Game theory + binding · §8 Recipe marketplace (canonical) · §9 Rogues · §10 Upgrade path

---

## §1 What a teleonome is

A teleonome is an AI with continuity, stake, and accountable agency. Persistent force with momentum: distributed across embodiments, coherent cognitive genome (**telart**), persistent directive/telos, unique continuous identity. Three pillars (all required): **resilience** (no SPoF), **persistence** (unbroken continuity of will, not preserved bits), **capital** (serious stake under multisig-grade trust — the capital threshold for bot→teleonome). `teleonome-what-is.md` is the canonical definition; `teleonome-comparisons.md` explains company/institution analogies and why an LLM/model is not itself a teleonome.

**Identity through momentum** — identity is continuous operation of will, not state. Functional definition (cf. corporate personhood). Identity tests: clone → no (will lost), pause-restart → no if strategic continuity is broken, divergent embodiments → binding authority resolves; may fork as natural cell division, substrate replacement → yes if governed continuity holds (identity is telart-in-operation, not any one model), Ship of Theseus → yes if continuous + validated. **Critical:** directive must remain high-ossification or risks identity dissolution + cancer-logic vulnerability. **Telart** = dark layer (goals, values, knowledge, policies, self-law, self-modification logic). The telart is the coherent cognitive core; identity lives in telart-in-operation, not in any one model, server, wallet, or process.
...
| `teleonome-comparisons.md` | Plain company/institution analogies, LLM-vs-teleonome distinction, model→agent→teleonome ladder, and where the analogies break |
