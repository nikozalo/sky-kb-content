...
Patterns use the [truth values](../core-concepts/truth-values.md) model — positive and negative evidence weights, not a single confidence score.

**Not this:** `(fork, effectiveness, high) @confidence=0.85`

**This:** `(fork, effectiveness) @pos_weight=850 @neg_weight=150`

Strength and confidence derive from weights, negative evidence is explicit, and [ossification](../core-concepts/ossification.md) emerges naturally (high total weight = hard to shift) — one mechanism within [synomic inertia](../core-concepts/synomic-inertia.md). Matches the full architecture's (strength, confidence) model.
...
The threat model is [cancer-logic](../core-concepts/cancer-logic.md) — internal drift, not external attackers. Bad patterns enter the KG, influence decisions, generate more bad patterns, and the system corrupts itself. Mitigations: high-weight patterns resist noise ([ossification](../core-concepts/ossification.md)), single observations can't corrupt established patterns, append-only enables rollback, and validation catches obvious drift.
...
- **How to evolve fitness functions beyond games** — for real-world domains, finding what to optimize for is itself a meta-level pattern-finding task (patterns for finding patterns for finding patterns). Games bootstrap the machinery with known fitness; the real value is when the system discovers its own evaluative criteria.
...
|----------|--------------|
| [`probabilistic-mesh.md`](../synodoxics/probabilistic-mesh.md) | Full truth value system these experiments build toward |
| [`synome-layers.md`](../macrosynomics/synome-layers.md) | The 5-layer architecture (synart, telart, embart) |
| [`dreamer-perspective.md`](../synoteleonomics/dreamer-perspective.md) | Full dreamer embodiment — the evolution target |
| [`security-and-resources.md`](../synodoxics/security-and-resources.md) | Security as self-corruption prevention |
| [`short-term-actuators.md`](../macrosynomics/short-term-actuators.md) | Parallel actuator pathway (teleonome-less beacons) |
