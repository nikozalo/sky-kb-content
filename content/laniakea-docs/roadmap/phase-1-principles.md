...
The principles below are distilled from the Phase 1 design decisions and the worked NFAT example in [`phase-1-spaces.md`](phase-1-spaces.md). They describe the invariants the Phase 1 substrate is built around — the discipline that lets later phases unfold capacity without rewriting what books rest on. Each principle survives every later phase; what unlocks (real auctions, factory stack, sentinel formations, multi-form risk framework) extends these without contradicting them.
...
2. **Use Stressed Pull-to-Par (SPTP) for asset pull-to-par horizon.** Stress modifier reflects historical worst-case prepayment / amortization slowdowns. (P1 carve-out: SPTP = remaining nominal term, no stress modifier yet.)
...
5. **SPTP determines structbook eligibility.** In P1, any funded position with defined SPTP routes to `structbook`; SDR allocation determines the matched portion, not membership.

6. **Matched positions are capitalized for default-CRR only.** Held-to-par matching covers spread, rate, and liquidity for the matched portion; it never removes default-CRR.

7. **Unmatched positions get forced-loss treatment.** Capital is `max(default-CRR, forced-loss-capital) + rate-CRR`, where `forced-loss-capital = spread-CRR + liquidity-CRR`.
...
17. **Derived atoms are frame-local; replay is canonical.** Synserv re-derives book state from input atoms each heartbeat. Derived values (`exobook-current-state`, `riskbook-default-crr`, `nfat-prime-projection`, `structbook-*`, `prime-trrc`, `prime-er`, …) live only within synserv's heartbeat frame as scratch. The frame is shared in-tick across cross-Space readers — synserv's heartbeat is the cross-book duality boundary, so a holding Prime's structbook sees the issuing halobook's projection in the same tick without persistence. For off-tick / historical inspection, replay from append-only input + rule atoms is the canonical mechanism. Any persistence layer (rolling-window cache, snapshots) is an implementation optimization, not part of the semantic model.
...
| Doc | Relationship |
|---|---|
| [`phase-1-spaces.md`](phase-1-spaces.md) | Phase 1 spec; the worked NFAT example exercises these principles end-to-end |
| [`roadmap-ideas.md`](roadmap-ideas.md) | Vocabulary, lift principle, sudo staircase that frames how these principles get extended |
| [`../roadstart/risk-framework.md`](../roadstart/risk-framework.md) | Risk-framework summary (these principles are the P1 instantiation of its substrate) |
